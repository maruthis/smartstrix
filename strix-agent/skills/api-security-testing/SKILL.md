...
Install, LLM setup, and full CLI flags are in the **penetration-testing-with-strix** skill. Read it if `strix --version` fails or the target is not an API.
...
- **Internal/private APIs:** run Strix on a host that can reach them (same Docker network, VPN, or `host.docker.internal`).
