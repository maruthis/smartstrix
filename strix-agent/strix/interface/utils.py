...
def clone_repository(
    ...
    # STRIX_REPOS_DIR overrides the OS temp dir. This matters when this
    # process runs inside a container that spawns SIBLING sandbox containers
    # via the host's Docker socket (saas/backend's real-scan mode): bind
    # mount sources are resolved by the host daemon, so a path under this
    # container's own /tmp is invisible to it. Setting STRIX_REPOS_DIR to a
    # path that's bind-mounted at the same absolute path on the host (see
    # docker-compose.yml) makes the clone visible to sibling containers too.
    repos_base = Path(os.environ.get("STRIX_REPOS_DIR") or tempfile.gettempdir()) / "strix_repos"
