...
def _apply_resource_limits(create_kwargs: dict[str, Any]) -> None:
    """Apply cgroup resource caps from the environment, with conservative
    defaults so autonomous sandboxes are not unbounded by accident. Set an
    env value to ``0``/``off``/``none``/``unlimited`` to opt out."""
    mem_limit = os.environ.get("STRIX_SANDBOX_MEM_LIMIT", "4g").strip()
    if mem_limit.lower() not in ("", "0", "off", "none", "unlimited"):
        create_kwargs["mem_limit"] = mem_limit

    shm_size = os.environ.get("STRIX_SANDBOX_SHM_SIZE", "1g").strip()
    if shm_size.lower() not in ("", "0", "off", "none", "unlimited"):
        create_kwargs["shm_size"] = shm_size

    cpus = os.environ.get("STRIX_SANDBOX_CPUS", "4").strip()
    if cpus.lower() not in ("", "0", "off", "none", "unlimited"):
        with contextlib.suppress(ValueError, OverflowError):
            nano_cpus = int(float(cpus) * 1_000_000_000)
            if 0 < nano_cpus <= 2**63 - 1:
                create_kwargs["nano_cpus"] = nano_cpus

    pids_limit = os.environ.get("STRIX_SANDBOX_PIDS_LIMIT", "1024").strip()
    if pids_limit.lower() not in ("", "0", "off", "none", "unlimited"):
