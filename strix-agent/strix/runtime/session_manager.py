...
_CAIDO_HOST_GATEWAY = "host.docker.internal"
...
def _resolve_caido_host(host: str) -> str:
    """Rewrite a loopback endpoint host to be reachable from this process.

    ``resolve_exposed_port`` reports the Docker daemon's own view of where a
    published port is bound, which defaults to ``127.0.0.1`` (or ``0.0.0.0``)
    when the port binding has no explicit HostIp. That address is loopback
    *on the machine running the daemon* — when this process is itself
    running inside the ``backend`` container (real-scan mode spawning
    sibling sandbox containers over the host's Docker socket), 127.0.0.1
    means the backend container, not the daemon's host, so the sandbox's
    published port is unreachable there. ``host.docker.internal`` is the
    same host-gateway hostname already used elsewhere in this codebase
    (docker_client.py, scan_setup.py) for the reverse direction; Docker
    Desktop resolves it in every container automatically, and Linux hosts
    get it via docker_client.py's ``extra_hosts`` on the sandbox side.

    Rewriting the URL's host alone isn't enough: Caido enforces a Host-header
    allowlist (DNS-rebinding protection, see containers/docker-entrypoint.sh)
    and returns 403 for a Host it doesn't recognize. The caller must also add
    this same hostname to ``STRIX_CAIDO_ALLOWED_DOMAINS`` in the sandbox's
    environment so caido-cli is started with a matching ``--ui-domain``.
    """
    if host not in ("127.0.0.1", "0.0.0.0", "localhost") or not _running_in_container():  # noqa: S104
        return host
    return _CAIDO_HOST_GATEWAY
...
def _cached_session_is_alive(bundle: dict[str, Any]) -> bool:
    """Return False when a cached Docker sandbox's container is gone or stopped.

    Non-Docker backends have no container to inspect, so they stay reusable.
    """
    ...
    except Exception:  # noqa: BLE001 - any inspect failure means don't reuse
        logger.warning("Cached sandbox container %s is gone", container_id)
        return False
    status = getattr(container, "status", None)
    if status != "running":
        logger.warning("Cached sandbox container %s status=%s", container_id, status)
        return False
    return True
...
async def create_or_reuse(
    ...
    *,
    ...
    if cached is not None:
        if _cached_session_is_alive(cached):
            logger.info("Reusing existing sandbox session for scan %s", scan_id)
            return cached
        logger.warning("Dropping dead cached sandbox session for scan %s", scan_id)
    ...
    # When this process itself runs in a container, resolve_exposed_port's
    # loopback host gets rewritten to _CAIDO_HOST_GATEWAY below — caido-cli
    # must be told to accept that Host header too, or it 403s every request
    # (see _resolve_caido_host).
    caido_allowed_domains = {"STRIX_CAIDO_ALLOWED_DOMAINS": _CAIDO_HOST_GATEWAY} if _running_in_container() else {}
    manifest = Manifest(
        entries=entries,
    ...
    caido_host = _resolve_caido_host(caido_endpoint.host)
    host_caido_url = f"{scheme}://{caido_host}:{caido_endpoint.port}"
