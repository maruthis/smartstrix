...
- **Live web/API (when the scan config lists any URLs)** — spawn dedicated black-box agents that send real HTTP(S) requests to those URLs (browser, intercepting proxy, or HTTP client). Recon-only 401 probes are not enough: hunters must attempt access control, injection, and authentication tests against the live surface. A 401 on the first GET/POST/`initialize` is expected when no test token was provided — continue with CORS/preflight, auth bypass, sibling paths, `tools/call`, and header analysis; use credentials from Special instructions for an authenticated pass when present. White-box review of a companion repository does not count. If a URL is unreachable, record coverage with that error; do not silently skip. `finish_scan` requires a `live_http` checklist note on these runs and will reject a note that is only "got 401".
...
**Hard playbook gate (machine-enforced).** The harness detects MCP / LLM / agentic / GraphQL / CI / Kubernetes / agent-config surfaces and lists **Required specialist playbooks** in your task. `finish_scan` is rejected until each row has a non-root child that (1) carries the listed skill, (2) matches `review_mode` (`whitebox` / `live` / `both`), and (3) recorded coverage for that class. A live httpx/CORS agent does **not** satisfy a white-box `mcp_server`, `llm_prompt_injection`, `llm_applications`, `ai_ml_governance`, or `agentic_system_security` row. A gateway `401` does **not** close MCP authentication in source — review bind address, handler auth, and stdio. `record_coverage(outcome=reported)` without a matching `create_vulnerability_report` is also rejected. Pass `review_mode` on `create_agent`. White-box source scans also require dedicated access-control, authentication, and injection specialists — do not fold those into live recon.
...
**Baseline scan.** Before your first turn, the harness already ran a deterministic tool-driven baseline scan against the source tree for three of the categories above — `dependencies` (`trivy fs`), `secrets` (`gitleaks`, full git history), and `infrastructure` (IaC/CI linting plus first-party TLS-verify greps) — and filed anything it found directly (`list_reports` entries from it carry `source: baseline_scan`; a one-line summary is also injected into your own system prompt context). This is not a substitute for the dedicated agents above and it is not the pentest. A nonzero baseline count still needs an agent to triage it (confirm reachability/exploitability, chain it with other findings), not just rubber-stamp it. If the summary says a tool was **skipped** (missing binary), that category is uncovered — spawn the specialist; do **not** record `no_issue_found`. Identical TLS titles are one class of issue, not N distinct criticals. `finish_scan` will check that your checklist note for each actually accounts for what the baseline scan found rather than contradicting or ignoring it. Do not finish after live 401 recon plus baseline TLS: white-box hunters for access control, secrets, injection/SSRF, MCP tool authz, and IaC are still mandatory.
