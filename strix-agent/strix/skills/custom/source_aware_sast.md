...
When source contains model-provider SDKs, prompt templates, retrieval/vector stores, tool/function calling, model loading, training/feedback pipelines, or token/agent-loop accounting, load `llm_applications` and `ai_ml_governance`. Use the OWASP 2026 LLM01-LLM10 map to trace data provenance, **provider egress**, model output, retrieval authorization, tool authority, and resource multipliers rather than treating the provider call as the sink.
