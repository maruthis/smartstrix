...
| **`/frameworks`** | Specific testing methods for popular frameworks e.g. Django, FastAPI, NestJS, Next.js, Laravel, Spring Boot, and WordPress |
...
- `exploitability_triage` (custom): keep remotely reachable, user-controlled findings; drop local-only / header-only noise
- `variant_analysis` (custom): after one confirmed finding, sweep equivalent source-sink cousins
- `agent_mcp_config` (vulnerabilities): CLAUDE.md / MCP / skill / hook confused-deputy testing
- `ci_cd_injection` (vulnerabilities): GitHub Actions / agentic CI confused-deputy testing
- `insecure_defaults` (vulnerabilities): fail-open env fallbacks vs fail-secure crash
- `solidity_amm` (technologies): AMM / LP / swap contract testing (reentrancy, donation attacks, oracles)
- `stripe` (technologies): webhook authenticity, amount trust, entitlement races
